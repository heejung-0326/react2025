...
function Form6(props) {
  ...
  return (
    <>
      ...
      <p>html태그에서는 textarea를 작성하고 안에 내용을 적지만 React jsx문법에서는 textarea value='함수명' 으로 작성해야한다.</p>
      <pre>
        *html문법 <textarea>내용적기</textarea><br />
        *jsx문법  text value=내용적기
      </pre>
      ...
      <ul>
        <li>console.log(menu) - useState상태 업데이트가 비동기적으로 처리되기때문에 이전값이 출력됨.</li>
        <li>수정할 부분 : setMenu(e.target.value); console.log(menu)</li>
        <li>개선된 방법1 : console.log(e.target.value);</li>
        <li>개선된 방법2 : useEffect함수를 사용<br /> 2번은 주석 처리한 내용 확인
        {/* 1번줄에 import useEffect from 'react'; 추가
        중간코드에 
        useEffect(()=>{
